...
##' @name TEnGExA
##' @rdname TEnGExA
##' @title Tissue-Enrichment Analysis of any Number of Genes and Tissues
#' @param x A numeric matrix or data frame of raw RNA-Seq read counts or fpkm values (file path).
#' @param fpkm_flag Numeric. 0 for count matrix or 1 for FPKM value matrix. Default-value 0
#' @param threshold Numeric. fpkm value threshold to be considered for calling any gene/transcript as expressed. Default-value 5
#' @param tissue_num Numeric. Minimum number of tissues to be considered for assigning group enrichment class. Default-value 5
#' @param min_fpkm Numeric. Minimum FPKM value threshold to be considered for expression analysis.  Default-value 1
#' # Default-output-file output1.csv
#' @importFrom utils head read.delim write.table write.csv
#' @importFrom data.table data.table
#' @export
#' 

TEnGExA <- function(x, fpkm_flag=0, threshold=5, tissue_num= 5, min_fpkm= 1) {
  ...
  ###################### Thershold ##########
  #threshold=5
  ########  number of tissue need to be considered for group enrichment class #
  #tissue_num= 5
  
  ########  FPKM value minimum threshold default 1 #
  #min_fpkm= 1
  ...
  {
    # stop("Data is double but you have chosen data is in counts")
    # write.csv("Data is double but you have chosen data is in counts",finalfile)
    write("Data is double but you have choosen data is in counts... ", file=logFile, append=TRUE)
  }
  
  if (is.integer(x1) && fpkm_flag==1)
  {
    # stop("Data is integer (Count) but you have chosen data is in FPKM")
    # write.csv("Data is integer (Count) but you have chosen data is in FPKM",finalfile)
  ...
  num_tissues <- c1 - u + 1  #Added: for number of tissue columns
  new_cols <- 2 * num_tissues   #Added : to ensure that new columns exist
  
  for (k in 1:new_cols) {       #Added: to add empty columns for results
    fpkm[[c1 + k]] <- NA        #Added: to initialize added columns with NA
  }
  ...
  for (i in 1:r1)
  {
    ...
    #maxfpkm=sort(fpkm[i,u:c1],decreasing = TRUE)   #Remove
    fpkm_row_vals <- unlist(fpkm[i, u:c1])          #Added
    maxfpkm <- sort(fpkm_row_vals, decreasing = TRUE) #Added
    ...
    for (j in 1:(t1-1))
    {
      ...
      {
        fpkm[i,c1+2*j]=names(maxfpkm)[j]    ## Replaced colnames(maxfpkm) with names(maxfpkm)
        if (j<=1)
        {flageg=1
        egstr<-paste("Tissue-Enriched:" ,names(maxfpkm)[j])       ## Replaced colnames(maxfpkm) with names(maxfpkm)
        }
        else
        {   if (flageg==0)
    ...
    fpkm[i,c1+2*(t1-1)+2]=names(maxfpkm)[1]       ## Replaced colnames(maxfpkm) with names(maxfpkm)
    
    if (fpkm[i,c1+2*(t1-1)+1]>=threshold  & flageg==0 & maxfpkm[1]>=min_fpkm )
    { egstr<-paste("Tissue-Enhanced:" ,names(maxfpkm)[1])       ## Replaced colnames(maxfpkm) with names(maxfpkm)
    flageg=1}
